...
export class RabbitMqEventProducer extends EventProducer {
  ...
  constructor(
    ...
    protected readonly config: RabbitMqEventProducerConfig,
    logger: EventLogger,
  ) {
    super(logger);
  }
    ...
    this.channel.on('close', () => {
      ...
      this.reconnectionPromise = new Promise((resolve, reject) => {
        ...
        setTimeout(
          ...
          this.config.reconnectionTimeoutMs ??
            RabbitMqEventProducer.DEFAULT_RECONNECTION_TIMEOUT_MS,
        );
      });
    });
